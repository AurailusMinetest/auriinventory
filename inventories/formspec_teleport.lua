function auriinventory.gen_formspec_teleport (player)
	local fs = [[
		size[17,8;]
	...
	if minetest.setting_getbool("temphome") then fs = fs .. "button[9,0.325;3.12,1;temphome;Set Temp Home]" end
	if minetest.setting_getbool("deathpoint") then fs = fs .. "button[9,1.225;3.12,1;deathpoint;Return to Death Point]" end
	if minetest.check_player_privs(player:get_player_name(), "teleport") then fs = fs .. "button[9,2.125;3.12,1;teleportto;Teleport to Player]" end
